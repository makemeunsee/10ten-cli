...
import { isOnlyDigits } from './char-range';
  ...
  if (candidate.reasons.length) {
    reason =
      ...
      candidate.reasons
        .map((reasonList) =>
          reasonList
            .map((reason) => deinflectL10NKeys[reason])
            .join(' < ')
        )
        .join('deinflect_alternate');
    if (showInflections) {
      reason += ` < ${input}`;
    }
  }
...
function entryMatchesType(entry: DictionaryWordResult, type: number): boolean {
  const hasMatchingSense = (test: (pos: PartOfSpeech) => boolean) =>
    entry.s.some((sense) => sense.pos?.some((pos) => test(pos as PartOfSpeech)));
