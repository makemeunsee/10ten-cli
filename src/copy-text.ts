...
const highPriorityLabelsSet = new Set(['i1', 'n1', 's1', 's2', 'g1']);

export function filterRelevantKanjiHeadwords(
...
export function filterRelevantKanaHeadwords(
...
export function serializeDefinition(
  ...
  if (senses.length > 1 && includeAllSenses) {
    const nativeSenses = senses
      .filter((s: any) => s.lang && s.lang !== 'en')
      .map(
        (s: any) => `• ${serializeSense(s, { includePartOfSpeech })}`
      );
...
function serializeSense(
  ...
  for (const g of sense.g) {
    ...
    if (g.type && g.type !== 'tm' && g.type !== 'none') {
      const glossTypeStr = `gloss type: ${g.type}`;
